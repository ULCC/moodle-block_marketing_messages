...
namespace block_marketing_messages\privacy;
...
/** @var string Flag used to determine if announcement is block-based or global */
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table(
            'block_marketing_messages',
            [
                'title' => 'privacy:metadata:block_marketing_messages:title',
                'message' => 'privacy:metadata:block_marketing_messages:message',
                'blockid' => 'privacy:metadata:block_marketing_messages:blockid',
                'deleted' => 'privacy:metadata:block_marketing_messages:deleted',
                'deleted_by' => 'privacy:metadata:block_marketing_messages:deleted_by',
                'created_by' => 'privacy:metadata:block_marketing_messages:created_by'
            ],
            'privacy:metadata:block_marketing_messages'
        );

        $collection->add_database_table(
            'block_marketing_messagesdiss',
            [
                'user_id' => 'privacy:metadata:block_marketing_messagesdiss:user_id',
                'not_id' => 'privacy:metadata:block_marketing_messagesdiss:not_id',
                'dismissed' => 'privacy:metadata:block_marketing_messagesdiss:dismissed',
                'seen' => 'privacy:metadata:block_marketing_messagesdiss:seen'
            ],
            'privacy:metadata:block_marketing_messagesdiss'
        );
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        // Get context IDs from block instance IDs of announcements user has seen.
        $contextlist->add_from_sql(
            "SELECT c.id
               FROM {context} c
               JOIN {block_marketing_messages} adv ON adv.blockid = c.instanceid
               JOIN {block_marketing_messagesdiss} advdis ON advdis.user_id = :userid
        ...
        $blockids = $DB->get_records_sql(
            "SELECT DISTINCT adv.blockid
               FROM {block_marketing_messages} adv
               JOIN {block_marketing_messagesdiss} advdis ON advdis.not_id = adv.id
        ...
        if (isset($blockids) && !empty($blockids)) {
            ...
            // If announcement set to display globally, system context is added.
        ...
        if ($context->contextlevel == CONTEXT_SYSTEM) {
            ...
            $userlist->add_from_sql('user_id',
                "SELECT advdis.user_id
                   FROM {block_marketing_messagesdiss} advdis
                   JOIN {block_marketing_messages} adv ON adv.id = advdis.not_id
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        /* TODO: Edge case if user has not seen the announcement... What about deleted_by or created_by?
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        if ($context->contextlevel == CONTEXT_SYSTEM) {
            $delrecords = $DB->get_records('block_marketing_messages',
                array('blockid' => SITE_NOTIFICATION),
                null,
                'id');
            ...
            // Get id of announcement data to delete based on provided context instance id.
    ...
    public static function adv_delete_user_data($userid) {
        // We won't delete announcement due to user data being deleted - just 'clear' user id.
        global $DB;

        // If user created announcement.
        $DB->set_field('block_marketing_messages', 'created_by', -1, array('created_by' => $userid));

        // If user deleted announcement.
        $DB->set_field('block_marketing_messages', 'deleted_by', -1, array('deleted_by' => $userid));

        // If user viewed/dismsissed announcement.
        $DB->delete_records('block_marketing_messagesdiss', array('user_id' => $userid));
    }

    /**
     * Delete user data generated/managed by the block - based on record passed.
     *
     * @param   int $recordid The recordid of the announcement which contains user data to be deleted.
     */
    public static function adv_delete_record_data($recordid) {
        ...
        // If user created announcement.
        $DB->set_field('block_marketing_messages', 'created_by', -1, array('id' => $recordid));

        // If user deleted announcement.
        $DB->set_field('block_marketing_messages', 'deleted_by', -1, array('id' => $recordid));

        // If user viewed/dismsissed announcement.
        $DB->delete_records('block_marketing_messagesdiss', array('not_id' => $recordid));
    }
}
